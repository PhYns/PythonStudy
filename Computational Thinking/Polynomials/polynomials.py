...
def generate_models(x, y, degs):
    ...
    models = list()
    for degree in degs:
        models.append(np.array(polyfit(x,y,degree)))
    return models
...
def r_squared(y, estimated):
    ...
    """
    dividend, divisor = float(0), float(0)
    yMean = np.mean(y)
    
    # Upper part
    for element in range(len(y)):
        dividend += (y[element]-estimated[element])**2
        
    # Lower part
    for element in range(len(y)):
        divisor += (y[element] - yMean)**2
    
    return float(1-(dividend/divisor))
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        rSquareds.append(r_squared(y,polyval(model,x)))
    
    # Choose the best curve
    bestModel = models[rSquareds.index(max(rSquareds))]
    
    # Plot
    pylab.plot(x, y, 'bo', label = 'Measured points')
    pylab.plot(x, polyval(bestModel,x), 'r', label = 'Best fit')
    pylab.title("Fit Curve with R^2 of " + str(round(max(rSquareds),3)))
    pylab.xlabel("Years")
    pylab.ylabel("Temperature in ºC")
